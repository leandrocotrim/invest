...
response_status_invest = requests.get(url_status_invest)
df_status_invest = pd.json_normalize(response_status_invest.json())

df_status_invest.set_index('ticker', inplace=True)
df_status_invest.index.name = None

## 

response_fundamentus = requests.get(url_fundamentus, headers=headers)
df_fundamentus = pd.read_html(response_fundamentus.content, attrs={'id': 'resultado'})[0]

to_float = lambda s: s.replace('%', '').replace(',', '.')
df_fundamentus['Div.Yield'] = df_fundamentus['Div.Yield'].apply(to_float).astype(float)

df_fundamentus.set_index('Papel', inplace=True)
df_fundamentus.index.name = None


# mo máximo 11 vezes o lucro, perspectiva de crescimento
# sem divida
# registros bons com mais de 2 anos
# All anos dy ou lucro
# ghraran